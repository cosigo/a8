...

{
  ...
  assert.match(q,/Quartz Prism Length &amp; Light Experiment v0\.2/);
  ...
  console.log('PASS 67 · quartz experiment v0.2 remains explicitly speculative and includes the reviewed comparator, vacuum-λ×f, and Rosetta boundaries');
}
{
  assert.match(readPublic('index.html'),/href="\/quartz\.html"[^>]*>OPEN QUARTZ \/ LENGTH EXPERIMENT/);
...
}
{
  ...
  assert.equal(sha256File(corePath),'d9e9d768becf1390a4550facb23fdd966d643209520a49a7c035c9fbfb56dbfe');
  console.log('PASS 87 · accepted v5.4.18 core hash is byte-for-byte unchanged by the static laboratory patches');
}
{
  const serverPath=path.join(__dirname,'..','server.js');
  const pkg=JSON.parse(fs.readFileSync(path.join(__dirname,'..','package.json'),'utf8'));
  assert.equal(sha256File(serverPath),'7520609f68151d24137730882fa90efad2396f1d1cbed9a77dccfc6c9e86373e');
  assert.equal(pkg.version,'5.4.18');
  console.log('PASS 88 · accepted v5.4.18 server hash and runtime package version remain unchanged — 88/88');
}

console.log('ALL 88 A8 v5.4.18 + STATIC PATCH TESTS PASSED');
