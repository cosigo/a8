...
function zeroMeridianMsSinceMidnight() {
  // Conventional UTC is used only as the implementation bridge
  // to the shared 0-meridian phase. Operator local timezone is ignored.
  const d = new Date();
...
$('alignNow').addEventListener('click', async () => {
  try {
    await send('alignLocal', {
      msSinceMidnight: zeroMeridianMsSinceMidnight(),
      source: 'ENGINEERING LAB · 0-MERIDIAN REFERENCE · UTC BRIDGE'
    });
  } catch (e) { alert(e.message); }
});
...
// Read-only diagnostic attachment.
// The recorder itself operates autonomously inside A8Core.
fetch('/api/recorder', {cache:'no-store'})
  .then(r => r.json())
  .then(renderRecorder)
  .catch(() => {});
})();
