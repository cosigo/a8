...
function laneState(
  ...
  return {
    ...
    exactResonanceMultiplier:
      m.toString(),

    seedFullOrbitRaw:
      String(
        seed.seedFullOrbitRaw
      ),

    seedNormalizedRaw:
      String(
        seed.seedNormalizedRaw
      ),

    normalizedResidualRaw:
      String(
        seed.normalizedResidualRaw
      ),

    liveDriverFullOrbitRaw:
      String(
        seed.liveDriverFullOrbitRaw
      ),
...
function buildState() {
  ...
  return {
    ...
    schema:
      'A8-LIVE-JOVIAN-PERIOD-DRIVER-V1',
    ...
    qualification: {
      ...
      /*
       * OPERATIONAL RULER POLICY
       *
       * The established Jovian 1:2:4 ruler is the operating
       * period reference. Fresh turn windows are independent
       * validation evidence, not a prerequisite for operation
       * and never silently retune the locked ruler.
       */
...
function emitNewPulses(
  ...
  for (
    ...
    if (
      ...
      /*
       * Normally one crossing at a time.
       * Limit journal replay if transport
       * returns after a long interruption.
       */
      let first =
        prior + 1n;
...
server.listen(
  ...
  () => {
    ...
    console.log(
      'PERIOD SOURCE · SEALED THREE-MOON 1:2:4 BOOTSTRAP'
    );
