...

{
  ...
  const phase = c.alignClockToUtcMilliseconds(12 * 60 * 60 * 1000, 'TEST NOON');
...

{
  ...
  assert.equal(pkg.version,'5.4.20');
  assert.equal(snap.version,'5.4.20');
  assert.match(index,/v5\.4\.20/);
  assert.match(index,/A8 Time Laboratory v5\.4\.20/);

  console.log('PASS 66 · v5.4.20 package, core snapshot, and primary engineering page identity agree');
}


// Static Patch 03: COSIGO shell + Quartz experiment acceptance tests.
// These tests extend the accepted v5.4.18 suite without changing recovery/core behavior.
const publicDir = path.join(__dirname,'..','public');
const shellPages = ['index.html','clock.html','moons.html','audio.html','data.html','telescope.html','angle.html','quartz.html'];
...
{
  const q=readPublic('quartz.html');
  assert.match(
    q,
    /<title>Auspicious 8 — Quartz Prism Length & Light Experiment v0\.4 \| COSIGO Labs<\/title>/
  );
  assert.match(
    q,
    /<h1>Quartz Prism Length &amp; Light Experiment<\/h1>/
  );
  assert.match(q,/experimental design, not an A8 definition/);
  assert.match(q,/Comparator independence boundary/);
  assert.match(q,/λ_q\(vac\)/);
  assert.match(q,/888 \/ 666/);
  console.log('PASS 67 · quartz experiment v0.4 remains explicitly speculative and includes the reviewed comparator, vacuum-λ×f, 3-D recovery, and Rosetta boundaries');
}
{
  assert.match(readPublic('index.html'),/href="\/quartz\.html"[^>]*>QUARTZ \/ LENGTH EXPERIMENT/);
  console.log('PASS 68 · engineering home links to the Quartz / Length experiment');
}
{
  assert.match(readPublic('angle.html'),/href="\/quartz\.html"[^>]*>QUARTZ \/ LENGTH EXPERIMENT/);
  console.log('PASS 69 · angle translator cross-links to the Quartz / Length experiment');
}
{
  const q=readPublic('quartz.html');
  assert.match(q,/href="\/"[^>]*>A8 LAB HOME/);
  assert.match(q,/href="\/angle\.html"[^>]*>A8 ANGLE TRANSLATOR/);
  console.log('PASS 70 · quartz experiment has explicit return routes to the lab and angle translator');
}
{
  for(const page of shellPages) assert.match(readPublic(page),/class="cosigo-global-nav"/);
  console.log('PASS 71 · all eight public lab pages carry the COSIGO global navigation shell');
}
{
  for(const page of shellPages) assert.match(readPublic(page),/class="cosigo-site-footer"/);
  console.log('PASS 72 · all eight public lab pages carry the common COSIGO disclaimer footer');
}
{
  for(const page of shellPages) assert.match(readPublic(page),/\/assets\/cosigo_master_512\.gif/);
  console.log('PASS 73 · every lab page uses the bundled local COSIGO icon rather than a remote rendering dependency');
}
{
  for(const page of shellPages) assert.match(readPublic(page),/href="https:\/\/cosigo\.io\/"/);
  console.log('PASS 74 · every lab page has a direct COSIGO Home exit');
}
{
  for(const page of shellPages) assert.match(readPublic(page),/href="\/"[^>]*>A8 LAB HOME/);
  console.log('PASS 75 · every lab page has an internal A8 Lab Home route');
}
{
  for(const page of shellPages) assert.match(readPublic(page),/href="https:\/\/a8\.cosigo\.io\/"/);
  console.log('PASS 76 · every lab page links to the public A8 instrument');
}
{
  for(const page of shellPages) assert.match(readPublic(page),/href="https:\/\/contact\.cosigo\.io\/"/);
  console.log('PASS 77 · every lab page exposes the COSIGO contact route');
}
{
  for(const page of shellPages) assert.match(readPublic(page),/href="https:\/\/github\.com\/cosigo"/);
  console.log('PASS 78 · every lab page exposes COSIGO GitHub');
}
{
  for(const page of shellPages) assert.match(readPublic(page),/href="https:\/\/cosigo\.io\/project-status\.html"/);
  console.log('PASS 79 · every lab footer links to Educational Purpose / Project Status');
}
{
  for(const page of shellPages) assert.match(readPublic(page),/href="https:\/\/github\.com\/cosigo\/cosigo\.io"/);
  console.log('PASS 80 · every lab footer links to the main-site source repository');
}
{
  for(const page of shellPages) assert.match(readPublic(page),/Research \/ engineering notice:/);
  console.log('PASS 81 · every lab footer identifies the material as research / engineering work');
}
{
  for(const page of shellPages) assert.match(readPublic(page),/Legacy \/ SI values are comparison coordinates where identified, not native A8 authority/);
  console.log('PASS 82 · every lab footer preserves the native-A8 versus legacy/SI authority boundary');
}
{
  for(const page of shellPages) {
    const html=readPublic(page);
    assert.match(html,/educational and experimental project/);
    assert.match(html,/Nothing in this laboratory is an investment offering/);
  }
  console.log('PASS 83 · every lab footer carries the educational/experimental and non-investment disclaimer');
}
{
  const icon=fs.readFileSync(path.join(publicDir,'assets','cosigo_master_512.gif'));
  assert(icon.length>1024,'bundled COSIGO icon should not be empty/truncated');
  assert.equal(icon.subarray(0,6).toString('ascii').startsWith('GIF'),true);
  console.log('PASS 84 · bundled COSIGO icon is present and has a valid GIF signature');
}
{
  const css=fs.readFileSync(path.join(publicDir,'site-shell.css'),'utf8');
  assert.match(css,/\.cosigo-global-nav/);
  assert.match(css,/\.cosigo-site-footer/);
  assert.match(css,/@media\(max-width:/);
  console.log('PASS 85 · shared shell CSS defines navigation, footer, and responsive behavior');
}
{
  const q=readPublic('quartz.html');
  assert.match(q,/href="https:\/\/cosigo\.io\/"/);
  assert.match(q,/href="https:\/\/contact\.cosigo\.io\/"/);
  assert.match(q,/href="https:\/\/github\.com\/cosigo"/);
  assert.match(q,/href="https:\/\/a8\.cosigo\.io\/"/);
  console.log('PASS 86 · the deepest rabbit-hole page — Quartz — has all four external escape routes');
}
{
  const corePath=path.join(__dirname,'..','core','a8-core.js');
  assert.equal(sha256File(corePath),'03fcd1f5bc90354e616d4787558a4bdfecd7a259ccc986e61668fc6a8104a699');
  console.log('PASS 87 · Gate-1 identity-stamped v5.4.20 core hash is byte-for-byte sealed');
}
{
  const serverPath=path.join(__dirname,'..','server.js');
  const pkg=JSON.parse(fs.readFileSync(path.join(__dirname,'..','package.json'),'utf8'));
  assert.equal(sha256File(serverPath),'fe87d2d47898c7ba03f8f8f2d84c69b1f1aa0593a657d8b28725840ff4fea4dd');
  assert.equal(pkg.version,'5.4.20');
  console.log('PASS 88 · Gate-4D Terra-Ship-Slip event-wiring server hash is sealed and runtime package identity is v5.4.20 — 88/88');
}

console.log('ALL 88 A8 v5.4.20 GATE-4D BASE REGRESSION TESTS PASSED');
