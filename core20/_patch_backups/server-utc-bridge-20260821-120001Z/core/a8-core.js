...
class A8Core {
  ...
  alignClockToLocalMilliseconds(msSinceMidnight, source = 'ONE-SHOT LOCAL WALL CLOCK') {
    const ms = Number(msSinceMidnight);
    if (!Number.isFinite(ms) || ms < 0 || ms >= 86400000) {
      throw new Error('local milliseconds since midnight must be 0..86399999.999');
    }
    ...
    this.lastAlign = {
      ...
      localMillisecondsSinceMidnight: ms,
      eventSeq: this.eventSeq + 1,
    };
    this.recordEvent(
      'ALIGN',
      `ONE-SHOT ALIGN · DAY_PHASE17 ${before.toString()} → ${phase.toString()}`,
      {
        source,
        localMillisecondsSinceMidnight: ms,
